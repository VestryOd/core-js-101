...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const delimiter = Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  const result = (x1 * x2 + y1 * y2) / delimiter;
...
function roundToPowerOfTen(num, pow) {
  const newNumber = num / 10 ** pow;
  return Math.round(newNumber) * 10 ** pow;
}
